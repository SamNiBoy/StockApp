...
where mx.id = mn.id
  and not exists (select 'x' from stkdat2 s1 where s1.id = mx.id and s1.ft_id > mx.ft_id and s1.cur_pri > 0)
  and not exists (select 'x' from stkdat2 s1 where s1.id = mn.id and s1.ft_id < mn.ft_id and s1.cur_pri > 0)
