...
public class StockObserverable extends Observable {
	...
	public void update() {
		...
		String sql = "select s1.id, " 
		        + "       s1.name,"
				+ "       avg((s2.cur_pri - s2.yt_cls_pri) / s2.yt_cls_pri) avgSpeed, "
				+ "       avg((s2.dl_stk_num - s3.dl_stk_num)/100) avgHand"
				+ "  from stk s1, stkdat2 s2, stkdat2 s3 "
				+ " where s1.id = s2.id "
				+ "   and s2.id = s3.id "
		...
		if (body.length() > 0) {
			...
			content = Summary + body + "<br/>" + Summary + body2;
			this.setChanged();
	...
	public void update1() {
		...
		Summary += "<table border = 1>" + "<tr>" + "<th> ID</th> " + "<th> Lst_Pri</th> " + "<th> Hst_pri</th> "
				+ "<th> Top3CNTPCT</th> " + "<th> CUR_PRI</th> " + "<th> BUCKET No.</th> </tr> ";
			...
			while (rs.next()) {
				...
				if (s != null) {
					if (s.getCur_pri() < lst + (hst - lst) * 3.0 / 10.0 && s.getCur_pri() >= lst && top3Pct > 0.6) {
						...
						body += "<tr> <td>" + s.getID() + "</td>" + "<td> " + nf.format(lst) + "</td>" + "<td> "
								+ nf.format(hst) + "</td>" + "<td> " + nf.format(top3Pct) + "%</td>" + "<td> "
								+ nf.format(s.getCur_pri()) + "</td>" + "<td> " + nf.format(BucketNo) + "</td></tr>";
					} else if (s.getCur_pri() < lst || s.getCur_pri() > hst) {
						if (s.getCur_pri() < lst && top3Pct > 0.6) {
							body += "<tr> <td>" + s.getID() + "</td>" + "<td> " + nf.format(lst) + "</td>" + "<td> "
									+ nf.format(hst) + "</td>" + "<td> " + nf.format(top3Pct) + "%</td>" + "<td> "
									+ nf.format(s.getCur_pri()) + "</td>" + "<td> " + -1 + "*</td></tr>";
						} else if (s.getCur_pri() > lst && top3Pct > 0.6) {
							body += "<tr> <td>" + s.getID() + "</td>" + "<td> " + nf.format(lst) + "</td>" + "<td> "
									+ nf.format(hst) + "</td>" + "<td> " + nf.format(top3Pct) + "%</td>" + "<td> "
									+ nf.format(s.getCur_pri()) + "</td>" + "<td> " + 11 + "*</td></tr>";
						}
	...
	private String checkStatusForStock(boolean gz_flg, double pctRt) {
		...
		summary += "<table border = 1>" + "<tr>" + "<th> PTC</th> " + "<th> PTO</th> " + "<th> P4C</th> "
				+ "<th> KDL</th> " + "<th> PRK </th> " + "<th> RKS</th> " + "<th> ID </th> " + "<th> Name </th> "
	...
	private String getFollowers() {
		...
		String fmsg = "同增股票<br/>";
		fmsg += "<table border = 1>" + "<tr>" + "<th> Stock</th> " + "<th> Name</th> " + "<th> CurPri</th> "
				+ "<th> CurPct</th> " + "<th> Followers+ </th> " + "<th> FollowersAvgPct+ </th> "
				+ "<th> Followers- </th> " + "<th> FollowersAvgPct- </th> </tr> ";
	...
	private String getDetQtyPlused() {
		...
		StockPlused += "<table border = 1>" + "<tr>" + "<th> Stock</th> " + "<th> Name</th> " + "<th> CurPri</th> "
	...
	private String getIndex() {
		...
		try {
			...
			String sql = "select count(case when cur_pri > td_opn_pri then 1 else 0 end) IncNum, "
					...
					+ " avg((cur_pri - td_opn_pri)/td_opn_pri) avgPct," + " sum(dl_mny_num) totDlMny,"
					+ " case when cur_pri > td_opn_pri then 1 " + "               when cur_pri < td_opn_pri then -1 "
					+ "               when cur_pri = td_opn_pri then 0 end catagory " + " from stkdat2 "
					+ " where td_opn_pri > 0 "
					...
					+ "               when cur_pri = td_opn_pri then 0 end" + " order by catagory asc";
			ResultSet rs = stm.executeQuery(sql);
		...
		nf.setMaximumFractionDigits(2);
		if (catagory != -2) {
			index += "<tr> <td>" + StkNum + "</td>" + "<td> " + TotInc + "</td>" + "<td> " + nf.format(AvgIncPct * 100)
					+ "%</td>" + "<td> " + nf.format(totIncDlMny / 100000000) + "亿</td>" + "<td> " + TotDec + "</td>"
					+ "<td> " + nf.format(AvgDecPct * 100) + "%</td>" + "<td> " + nf.format(totDecDlMny / 100000000)
					+ "亿</td>" + "<td> " + TotEql + "</td>" + "<td> " + nf.format(totEqlDlMny / 100000000) + "亿</td>"
					+ "<td> "
					+ nf.format((TotInc * AvgIncPct + TotDec * AvgDecPct) * 100.0 / (TotInc * 0.1 + TotDec * 0.1))
					+ " C</tr></table>";
		}
