	...
	public static List<TradeRecord> readTradeRecords() {
		...
		try {
			...
			String sql = "select c.acntid, "
					...
					+ "left join usrstk u "
					+ "  on s.id = u.id "
					+ " order by c.add_dt desc, stkid, acntid, seqnum desc";
		...
		try {
			...
			if (rs.next()) {
				...
				if (hasusrstkflg == 1) {
			    	...
			    	sql = "insert into usrStk values ('osCWfs-ZVQZfrjRK0ml-eEpzeop0','" + stkid + "',1,0,0,1,'SYSTEM_GRANTED_TRADER','Manual','User manual suggested', 0,sysdate(), sysdate())";
			    }
